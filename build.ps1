...
try {
    ...
    Copy-Item -LiteralPath .\README.md,.\manifest.json,.\icon.png -Destination .\dist\package
    Compress-Archive -Path .\dist\package\* -DestinationPath .\dist\PlayerInventoryStacks-1.0.0.zip -Force
    Copy-Item -LiteralPath .\bin\Release\net472\PlayerInventoryStacks.dll -Destination .\dist\PlayerInventoryStacks.dll
    New-Item -ItemType Directory -Force -Path .\dist\source\src,.\dist\source\tests | Out-Null
    ...
    Copy-Item -LiteralPath .\PlayerInventoryStacks.csproj,.\build.ps1,.\README.md,.\manifest.json,.\icon.png -Destination .\dist\source
    Compress-Archive -Path .\dist\source\* -DestinationPath .\dist\PlayerInventoryStacks-1.0.0-source.zip -Force
    Get-FileHash -Algorithm SHA256 -LiteralPath .\dist\PlayerInventoryStacks-1.0.0.zip,.\dist\PlayerInventoryStacks.dll,.\dist\PlayerInventoryStacks-1.0.0-source.zip |
